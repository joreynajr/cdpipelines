...
class JobScript:
	...
	def awk_trim(
		...
		line = 'zcat {} | '.format(fastq)
		line += "awk 'BEGIN{count=0} {count++ 1; if (count % 4 == 2 || count % 4 == 0) print(substr(\$0,1,100)); else print(\$0)}' \ "
		line += '> {}'.format(out)
		line = 'gzip {}'.format(out)
		if bg:
			line += ' &\n\n'
		else:
			line += '\n\n'
	...
	def cutadapt_pe_trim(
		...
		try:
			...
			raise Exception('Need to specify a correct path to D50 and D70 adapter file.')

		lines = 'cutadapt -m {} -a {} -A  {} -o {} -p {} -O {} {} {}'.format(min_read_len, d70_adapter, d50_adapter, r1_out, r2_out, overlap, r1, r2)
		if bg:
			lines += ' &\n\n'
		else:
			lines += '\n\n'
	...
	def combine_fastqs(
		...
		if scale_down: # Scaled down for testing. 
			out_fastq = out_fastq.replace('.gz', '')
			lines = 'zcat \\\n\t{} \\\n\t | head -n 1000 > {}'.format(' \\\n\t'.join(fastqs), out_fastq)
			lines += '\nbgzip {}'.format(out_fastq)
			out_fastq = out_fastq + '.gz'
		else:
			if len(fastqs) > 1:
				lines = 'cat \\\n\t{} \\\n\t> {}'.format(' \\\n\t'.join(fastqs), out_fastq)
			else:
