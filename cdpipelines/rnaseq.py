...
def pipeline(
	...
	combined_r1 = job.combine_fastqs(r1_fastqs, suffix='R1', bg=True, scale_down=scale_down)
	...
	##### Job 4: Run sample identity check. #####
	job = RNAJobScript(
		sample_name,
	...
	##### Job 6: Make md5 hash for final bam file. #####
	job = RNAJobScript(
		sample_name, 
		job_suffix='md5',
		outdir=os.path.join(outdir, 'alignment'), 
	...
	##### Job 7: Make bigwig files for final bam file. #####
	job = RNAJobScript(
		sample_name, 
		job_suffix='bigwig',
		outdir=os.path.join(outdir, 'alignment'), 
	...
	##### Job 8: Get featureCounts and DEXSeq counts. #####
	job = RNAJobScript(
		sample_name, 
		job_suffix = 'counts',
...
def trim_pipeline(
	...
	overlap : int
		The minimum amount of overlap required for adapter removal.  

	min_read_len : int 
		The minimum number of base-pairs each read of a read-pair need to be 
		kept after cutadapt trimming.
	...
	if r2_fastqs is None:
		...
		r2_fastqs = [os.path.realpath(x) for x in r2_fastqs]
		combined_r2 = job.combine_fastqs(r2_fastqs, suffix='R2', bg=True, scale_down=scale_down)
		combined_r2 = job.add_output_file(combined_r2)
	with open(job.filename, "a") as f:
			f.write('\nwait\n\n')

	# Trim reads. 
	r1_out = combined_r1.replace('_combined_', '_trimmed_')
	r2_out = combined_r2.replace('_combined_', '_trimmed_')

	if overlap > 0: 
		trimmed_r1, trimmed_r2 = job.cutadapt_pe_trim(
			combined_r1,
			combined_r2,
			r1_out,
			r2_out,
			d50_adapter=d50_adapter_fn,
			d70_adapter=d70_adapter_fn,
			overlap=overlap,
			min_read_len=min_read_len)
		with open(job.filename, "a") as f:
				f.write('\nwait\n\n')
	
		input_r1, input_r2 = trimmed_r1, trimmed_r2

	else: 
		input_r1, input_r2 = combined_r1, combined_r2
		

	# Align reads.
	(star_bam, log_out, log_final_out, log_progress_out, sj_out, 
	 transcriptome_bam) = \
			job.star_align(input_r1, input_r2, rgpl, rgpu, star_index,
							job.threads, genome_load=star_genome_load)
	star_bam = job.add_output_file(star_bam)
	transcriptome_bam = job.add_output_file(transcriptome_bam)
	log_final_out = job.add_output_file(log_final_out)
	...
	# Input files.
	job.add_input_file(input_r1, delete_original=True)
	job.add_input_file(input_r2, delete_original=True)

	# Run fastQC.
	if input_r2 is not None:
		fastqc_html, fastqc_zip = job.fastqc([input_r1, input_r2],
											 fastqc_path)
	else:
		fastqc_html, fastqc_zip = job.fastqc([input_r1, ''],
											 fastqc_path)
	[job.add_output_file(x) for x in fastqc_html + fastqc_zip]
	...
	##### Job 12: Run sample identity check. #####
	#print('Job 12: Run sample identity check.')
	job = RNAJobScript(
		sample_name,
		job_suffix='sample_identity_check',
		outdir=os.path.join(outdir, 'qc'),
