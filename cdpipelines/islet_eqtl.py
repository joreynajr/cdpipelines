...
class Islet_eQTLJobScript(JobScript):
	...
	def gzip_fastq(self, 
		...
		-------
		r1_fastq : str
			Path to r1 fastq gz file.
		r2_fastq : str
			Path to r2 fastq gz file.
		"""

		lines = []
		if paired:
			cmd = 'gzip {}; gzip {}'.format(r1_fastq, r2_fastq)
			lines = self._add_execution_date(lines, cmd)
			with open(self.filename, 'a') as f:
				f.write('\n'.join(lines))
			return (r1_fastq + '.gz' , r2_fastq + '.gz')

		else:
			cmd = 'gzip {}'.format(r1_fastq)
			lines = self._add_execution_date(lines, cmd)
			with open(self.filename, 'a') as f:
				f.write('\n'.join(lines))
			return (r1_fastq + '.gz',)
...
def pipeline(
	...
	job.add_input_file(bam)
	

	# sorting bam by read name. 
	bam_read_srt = job.sambamba_sort(bam, queryname=True)

	# Converting bam to fastq. 
	r1_fastq, r2_fastq = job.bedtools_bamtofastq(bam_read_srt, paired=True)
	r1_fastq, r2_fastq = job.gzip_fastq(r1_fastq, r2_fastq, paired=True)
	for fq in [r1_fastq, r2_fastq]:
		job.add_output_file(fq)

	# Align reads.
	(star_bam, log_out, log_final_out, log_progress_out, sj_out, 
	 transcriptome_bam) = \
			job.star_align(r1_fastq, r2_fastq, rgpl, rgpu, star_index,
	...
	# Run fastQC.
	fastqc_html, fastqc_zip = job.fastqc([r1_fastq, r2_fastq],
										 fastqc_path)
