...
def pipeline(
	...
	combined_r2 = job.combine_fastqs(r2_fastqs, suffix='R2', bg=True) if paired_end else ''
	with open(job.filename, "a") as f:
			f.write('\nwait\n\n')
	# We don't want to keep the fastqs indefinitely, but we need them for the
	# fastQC step later.
	combined_r1 = job.add_output_file(combined_r1)
	combined_r2 = job.add_output_file(combined_r2) if paired_end else ''
	...
	job.add_input_file(combined_r2, delete_original=True) if paired_end else 'pass'
